...
class NetworkSession{
	...
	private function onSessionStartSuccess() : void{
		...
		$this->setHandler(new LoginPacketHandler(
			...
			function(bool $isAuthenticated, bool $authRequired, Translatable|string|null $error, ?string $clientPubKey) : void{
	...
	private function setAuthenticationStatus(bool $authenticated, bool $authRequired, Translatable|string|null $error, ?string $clientPubKey) : void{
		...
		if($error !== null){
			$lang = $this->server->getLanguage();
			$errorMsg = $error instanceof \pocketmine\lang\Translatable ? $lang->translate($error) : $error;
			$this->disconnect($lang->translate(KnownTranslationFactory::pocketmine_disconnect_invalidSession($errorMsg)));
